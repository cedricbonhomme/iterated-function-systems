...
- `draw fs n` — runs the chaos game into the 400x640 Graphics window (opened lazily on first draw, or explicitly with `init ()`).
- `render ?width ?height fs n` — runs it into an off-screen bitmap; `save_png ?width ?height fs n file` writes that bitmap as an 8-bit grayscale PNG (self-contained writer using stored deflate blocks — needs no display and no image library).
