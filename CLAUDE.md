...
- `load_fractint ?samples ?margin ?aspect file` / `parse_fractint` — read Fractint `.ifs` files, returning `(name, ifs)` pairs. Note the coefficient order differs from `kf`: a file's `a b c d e f` maps to `[|a; b; e; c; d; f|]`. Plain weights become cumulative `pb` thresholds (weighted by `|det|` when the file gives none), and `fit ?samples ?margin ?aspect fs` derives the viewport by sampling the attractor, since the format has none. Errors raise `Ifs_error`; 3D entries (13 numbers) are rejected. `example/classics.ifs` is a sample file.
