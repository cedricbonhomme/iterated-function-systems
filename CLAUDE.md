...
- The rest of the file is predefined `ifs` values: `barnsley`, `sierpinski`, `dragon`, `coral`, `tree`, `star`, `zigzag`, `crystal`, `binary`, `galaxy`, `koch`, `maple`, `fiddlehead`, `sunflower`, `lace`, `vegvisir`. To add a fractal, define a new `ifs` record following the same pattern (cumulative probabilities ending at 1.0, viewport chosen to frame the attractor).
